...
### Part h
We needed a loop for part h to incorporate x12 into into the data frame, but we could have obtained the column alone using vectorization since all columns in a data frame are vectors: 
```{r}
(df$x12 = df$x1 * df$x2)
```

### Part i
Similarly to part h, we could have created the column of total.score independent of the data frame by using vectorization since columns x1, x2, and x3 are, themselves, vectors:
```{r}
(df$total.score = (df$x1*2)+df$x2-df$x3)
```

### Part j
part j also could have been completed using vectorization in a slightly different manner. Using ifelse() instead of if() and sapply(), produces the same result as above. The difference is the ifelse() function is vectorized (applies to every entry in a vector; does not go by element by element):
```{r}
(evaluate.row = ifelse(df$x1 < 4, "low",
...
**Solution**
```{r}
instructions = html_element(instructions.section, "li")
```
